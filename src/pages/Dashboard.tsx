  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3">
        <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-4 flex-1 min-w-[150px]"><div className="text-xs text-[#5A6080] mb-1">隞閮?</div><div className="text-2xl font-bold text-[#4D9FFF]" style={M}>{totalSig}</div><div className="text-[10px] text-[#3D4560]">瘥?5 ????</div></div>
        <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-4 flex-1 min-w-[150px]"><div className="text-xs text-[#5A6080] mb-1">??</div><div className="text-2xl font-bold" style={{...M, color: s.winRate >= 50 ? '#00FFA3' : '#FFB800'}}>{s.totalSignals > 0 ? s.winRate + '%' : '??}</div><div className="text-[10px] text-[#3D4560]">{s.totalSignals > 0 ? `${s.wins}??${s.losses}鞎 : '蝑?鈭斗?閮?'}</div></div>
        <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-4 flex-1 min-w-[150px]"><div className="text-xs text-[#5A6080] mb-1">???</div><div className="text-2xl font-bold text-[#00FFA3]" style={M}>{s.totalSignals > 0 ? s.profitFactor : '??}</div><div className="text-[10px] text-[#3D4560]">PF = ?/?扳?</div></div>
        <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-4 flex-1 min-w-[150px]"><div className="text-xs text-[#5A6080] mb-1">蝮賣???/div><div className="text-2xl font-bold" style={{...M, color: s.totalPnl >= 0 ? '#00FFA3' : '#FF4D4D'}}>{s.totalSignals > 0 ? (s.totalPnl >= 0 ? '+' : '') + s.totalPnl + '%' : '??}</div><div className="text-[10px] text-[#3D4560]">璅⊥鈭斗?銝?/div></div>
      </div>

      {/* Market bias - connected to backend /api/market-bias */}
      <div className="bg-[#111827]/50 border border-[#1F2937] rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-medium text-[#8B95B0]">撣??</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-4">
          <div className="flex items-center justify-between mb-3"><h3 className="text-sm text-[#8B95B0]">? ??啣撥閮?</h3><button onClick={()=>navigate('/signals')} className="text-xs text-[#4D9FFF]">?亦??券 ??/button></div>
          {backendSignals.filter((s:any)=>s.score>=6).length === 0 && signals.filter(s=>s.scoreLabel==='strong').length === 0 ? (
            <div className="text-center py-8">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#4D9FFF]/10 text-[#4D9FFF] text-xs mb-2"><span className="w-2 h-2 rounded-full bg-[#4D9FFF] animate-pulse"/><span>??銝?/span></div>
              <p className="text-xs text-[#3D4560] mt-2">蝑?蝚血? 13 ??隞嗥?撘瑁???..</p>
            </div>
        ...
        {/* Smart Money */}
        <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-4">
          <div className="flex items-center justify-between mb-3"><h3 className="text-sm text-[#8B95B0]">?? ?唳??Ｗ???/h3><button onClick={()=>navigate('/smartmoney')} className="text-xs text-[#B76FFF]">?亦??券 ??/button></div>
          {(mockSM as any[]).map((tx:any) => (
            <div key={tx.id} className="flex items-center gap-2 py-2 border-b border-[#1F2937] last:border-0">
              ...
              <div className="flex-1 min-w-0"><div className="text-xs text-white truncate">{tx.walletLabel}</div><div className="text-[10px] text-[#5A6080]" style={M}>{tx.direction==='bullish'?'???方疏':'??鞈??'} ?? {tx.token} ({tx.usdFormatted})</div></div>
              <span className="text-[10px] text-[#3D4560]">{tx.minutesAgo}m</span>
            </div>
          ))}
          {smartMoneyFeed.length===0 && <p className="text-[10px] text-[#3D4560] text-center mt-1 italic">蝷箇??豢?</p>}
        </div>

        {/* Alerts */}
        <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-4">
          <div className="flex items-center justify-between mb-3"><h3 className="text-sm text-[#8B95B0]">?? ??啗郎??/h3><button onClick={()=>navigate('/alerts')} className="text-xs text-[#FFB800]">?亦??券 ??/button></div>
          {nextEvent && <div className="bg-[#FFB800]/10 border border-[#FFB800]/20 rounded-lg p-3 mb-3"><div className="text-xs font-bold text-[#FFB800]">?? {nextEvent.name} ???? {nextEvent.days} 憭?/div><div className="text-[10px] text-[#5A6080]">{nextEvent.date}</div></div>}
          ...
          {displayAlerts.length===0 && !nextEvent && <div className="text-center py-6"><span className="text-2xl">??</span><p className="text-xs text-[#3D4560] mt-2">?怎霅血</p></div>}
        </div>
      </div>

      {/* Status bar */}
      <div className="bg-[#0D1117] border border-[#1F2937] rounded-lg px-4 py-2 flex flex-wrap items-center gap-4">
        <span className="text-[10px] text-[#3D4560] mr-2">蝟餌絞???/span>
        <StatusDot label="Binance WS" active={binWs} />
        <StatusDot label="敺垢 WS" active={wsConnected} />
        <StatusDot label="PostgreSQL" active={health?.ready === true} />
        <StatusDot label="閮???" active={health?.ready === true} />
