...
export default function SmartMoneyPage() {
  ...
  const feed = smartMoneyFeed.length > 0 ? smartMoneyFeed : MOCK_TXS;
  const isReal = smartMoneyFeed.length > 0;

  // Calculate consensus per coin from feed
  const consensus = ['BTC','ETH','SOL','BNB','XRP','DOGE'].map(token => {
    const txs = feed.filter((t: any) => t.token === token);
    const bull = txs.filter((t: any) => t.direction === 'bullish').reduce((s: number, t: any) => s + (t.usdValue || 0), 0);
    const bear = txs.filter((t: any) => t.direction === 'bearish').reduce((s: number, t: any) => s + (t.usdValue || 0), 0);
    const total = bull + bear;
    const dir = total === 0 ? 'neutral' : bull > bear * 1.5 ? 'bullish' : bear > bull * 1.5 ? 'bearish' : 'neutral';
    const conf = total > 0 ? Math.min(100, Math.round(Math.max(bull, bear) / total * 100)) : 0;
    const ticker = tickers.find(t => t.name === token);
    return { token, dir, conf, bull, bear, net: bull - bear, price: ticker?.price || 0 };
  });
    ...
    <div className="space-y-4">
      ...
      <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          {!isReal && <span className="text-[10px] text-[#3D4560] italic">示範數據 · 等待 Etherscan API</span>}
        </div>
            ...
            <div key={tx.id} className="flex items-center gap-3 py-2.5 border-b border-[#1F2937] last:border-0 hover:bg-[#1F2937]/30 rounded transition-all" style={{ borderLeftWidth: 3, borderLeftColor: tx.direction === 'bullish' ? '#00FFA3' : '#FF4D4D' }}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] px-1.5 py-0.5 rounded" style={{ background: (tx.direction === 'bullish' ? '#00FFA3' : '#FF4D4D') + '15', color: tx.direction === 'bullish' ? '#00FFA3' : '#FF4D4D' }}>
                    {tx.direction === 'bullish' ? '←交易所 吸籌' : '→交易所 賣壓'}
                  </span>
                </div>
              ...
              <div className="text-right flex-shrink-0 pr-2">
                <div className="text-sm font-bold text-white" style={M}>{tx.usdFormatted}</div>
